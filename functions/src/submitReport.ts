...
class Report {
  constructor(
      @Field('temporary_contact_key_bytes') public temporary_contact_key_bytes:
          string, @Field('memo_data') public memo_data: string,
...
export const submitReportHandler = function(
    db: firestore.Firestore, request: functions.https.Request,
  ...
  try {
    ...
    jsonObject['timestamp'] =
        firestore.FieldValue.serverTimestamp()  // add server side timestamp

    // convert base64 strings to bytes
    jsonObject['temporary_contact_key_bytes'] =
        Buffer.from(jsonObject['temporary_contact_key_bytes'], 'base64')

    jsonObject['memo_data'] = Buffer.from(jsonObject['memo_data'], 'base64')

    jsonObject['signature_bytes'] =
        Buffer.from(jsonObject['signature_bytes'], 'base64')

    jsonObject['report_verification_public_key_bytes'] = Buffer.from(
        jsonObject['report_verification_public_key_bytes'], 'base64')
